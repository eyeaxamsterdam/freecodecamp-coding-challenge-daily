...
function wiseSpeak(sentence) {
    ...
    const idx = words.findIndex((w) => triggers.includes(w.toLowerCase()));

    const moved = words.slice(0, idx + 1).map((w) => w.toLowerCase());
    const remaining = words.slice(idx + 1);
    const capitalized = remaining[0][0].toUpperCase() + remaining[0].slice(1);

    return [capitalized, ...remaining.slice(1)].join(' ') + ', ' + moved.join(' ') + punctuation;
}
